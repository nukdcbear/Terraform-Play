...
locals {
  ...
  kv_short_name = var.owner == "Binford Tools" ? "${var.app}-${var.environment}-kv" : length(
    "${var.app}-${var.environment}-${replace(var.owner, " ", "-")}-kv",
    ) > 24 ? "${substr(var.app, 0, min(length(var.app), 7))}-${var.environment}-${substr(
      replace(var.owner, " ", "-"),
      0,
      min(length(replace(var.environment, " ", "-")), 7),
  )}-kv" : "${var.app}-${var.environment}-${replace(var.owner, " ", "-")}-kv"
